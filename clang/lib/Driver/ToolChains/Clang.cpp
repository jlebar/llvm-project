...
static void RenderFloatingPointOptions(const ToolChain &TC, const Driver &D,
  ...
  // Math functions cannot set errno on NVPTX: there is no errno, and CUDA's
  // libdevice does not provide the errno-setting libm functions that math
  // builtins are lowered to calls of under -fmath-errno, so such calls fail
  // to resolve at device link time. Ignore -fmath-errno.
  if (MathErrno && TC.getTriple().isNVPTX()) {
