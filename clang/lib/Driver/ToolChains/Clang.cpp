...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (IsCuda || IsHIP) {
    ...
    // Similarly, single-precision division on NVPTX is IEEE round-to-nearest
    // by default (matching nvcc's -prec-div=true); -ffast-math flips that
    // default and -f[no-]gpu-prec-div overrides it either way.
    if (!Args.hasFlag(options::OPT_fgpu_prec_div,
                      options::OPT_fno_gpu_prec_div, !UseFastMath))
      CmdArgs.push_back("-fno-gpu-prec-div");
  } else {
    Args.claimAllArgs(options::OPT_fgpu_approx_transcendentals,
                      options::OPT_fno_gpu_approx_transcendentals,
                      options::OPT_fgpu_prec_div,
                      options::OPT_fno_gpu_prec_div);
  }
