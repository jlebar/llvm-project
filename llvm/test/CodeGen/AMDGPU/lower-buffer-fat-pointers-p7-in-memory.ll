...
;; The two accesses below read the same field: ptr addrspace(7) has 256-bit
;; ABI alignment, so field 1 of %struct.has_p7 lives at byte offset 32, and
;; instcombine folds constant struct GEPs to i8 GEPs with that offset baked in
;; long before this pass runs. Both functions must load at byte offset 32.
...
; CHECK-SAME: (ptr addrspace(5) [[AGG:%.*]]) #[[ATTR0]] {
...
; CHECK-SAME: (ptr addrspace(5) [[A:%.*]], ptr addrspace(5) [[B:%.*]]) #[[ATTR0]] {
...
; CHECK-SAME: (ptr addrspace(5) [[P:%.*]], i32 [[I:%.*]]) #[[ATTR0]] {
