...
; GlobalISel reads the shared-memory aperture with a 64-bit copy of
; src_shared_base into an SGPR pair. When the value is then needed in VGPRs,
; MachineCopyPropagation used to forward src_shared_base directly into the
; VGPR pair copy, which copyPhysReg cannot decompose (the aperture registers
; have no addressable high half) on targets without v_mov_b64.
