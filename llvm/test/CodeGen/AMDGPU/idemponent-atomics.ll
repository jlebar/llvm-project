...
; An idempotent atomicrmw must not be lowered to a plain atomic load. Unlike a
; load, the write half of an atomicrmw participates in the location's
; modification order: two concurrent idempotent atomicrmws to the same address
; are totally ordered and one reads-from the other, which is what makes
; fence-based synchronization through them work. See issues #101551 and
; #56450.
...
; One thread of the two-thread store-buffering litmus from issue #101551 (the
; other thread does the same with %y and %z swapped): with the rmw lowered to
; a load, nothing in this function is a write to %x anymore, the fences no
; longer synchronize, and the forbidden "both threads load 0" outcome becomes
; observable.
...
; Sub-word idempotent cases used to crash isel ("Cannot select: AtomicLoadAdd
; (s16)"): the or/xor/sub 0 -> add 0 rewrite fired on types the add can't
; select. or/xor now widen to an i32 operation on the containing word; sub
; takes the cmpxchg expansion.
...
define i16 @global_agent_release_idempotent_or_i16(ptr addrspace(1) %in) {
; OPT-LABEL: define i16 @global_agent_release_idempotent_or_i16(
; OPT-SAME: ptr addrspace(1) [[IN:%.*]]) {
; OPT-NEXT:  [[ENTRY:.*:]]
; OPT-NEXT:    [[ALIGNEDADDR:%.*]] = call ptr addrspace(1) @llvm.ptrmask.p1.i64(ptr addrspace(1) [[IN]], i64 -4)
; OPT-NEXT:    [[TMP0:%.*]] = ptrtoint ptr addrspace(1) [[IN]] to i64
...
; OPT-NEXT:    [[MASK:%.*]] = shl i32 65535, [[SHIFTAMT]]
; OPT-NEXT:    [[INV_MASK:%.*]] = xor i32 [[MASK]], -1
; OPT-NEXT:    [[TMP2:%.*]] = atomicrmw add ptr addrspace(1) [[ALIGNEDADDR]], i32 0 syncscope("agent-one-as") release, align 4
; OPT-NEXT:    [[SHIFTED:%.*]] = lshr i32 [[TMP2]], [[SHIFTAMT]]
; OPT-NEXT:    [[EXTRACTED:%.*]] = trunc i32 [[SHIFTED]] to i16
; OPT-NEXT:    ret i16 [[EXTRACTED]]
;
; GFX942-LABEL: global_agent_release_idempotent_or_i16:
; GFX942:       ; %bb.0: ; %entry
; GFX942-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GFX942-NEXT:    v_mov_b32_e32 v2, v0
; GFX942-NEXT:    v_and_b32_e32 v0, -4, v2
...
; GFX942-NEXT:    s_setpc_b64 s[30:31]
entry:
  %val = atomicrmw or ptr addrspace(1) %in, i16 0 syncscope("agent-one-as") release, align 2
  ret i16 %val
}
