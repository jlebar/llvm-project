...
; The LiveRegOptimizer used to insert the coercion for a def at
; std::next(Def->getIterator()), which for a value-producing terminator
; (callbr, invoke) is past the end of the block, producing IR without a
; terminator. Such defs must simply be left un-coerced.
...
; The callbr result feeds a phi whose other incoming value is coercible; the
; new phi must be unwound and the original one kept.
...
; A callbr result feeding a loop-carried phi that also feeds itself: the
; unwound replacement phis form a use cycle, which must not keep them alive.
...
; A callbr result may also be used on an indirect edge, so there is no single
; block whose start the coercion could be inserted at.
...
; A use in a landingpad block: the conversion back to the original type must
; be inserted after the landingpad instruction, not before it.
