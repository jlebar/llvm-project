...
; Cannot fold to fract: fract is nan at +/-inf, but this returns the clamp
; constant there, and no infinity check discards the difference.
...
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    [[UNO:%.*]] = fcmp uno float [[X]], 0.000000e+00
; IR-FRACT-NEXT:    [[COND:%.*]] = select i1 [[UNO]], float [[X]], float [[MIN]]
; IR-FRACT-NEXT:    store float [[FLOOR]], ptr addrspace(1) [[IP]], align 4
...
; IR-FRACT-NEXT:  [[ENTRY:.*:]]
; IR-FRACT-NEXT:    [[FLOOR:%.*]] = tail call float @llvm.floor.f32(float [[X]])
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub nsz float [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    ret float [[MIN]]
;
...
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub <2 x float> [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call <2 x float> @llvm.minnum.v2f32(<2 x float> [[SUB]], <2 x float> splat (float f0x3F7FFFFF))
...
; Cannot fold: ninf on the min proves nothing about x. Its operands,
; x - floor(x) and the constant, are never infinite for any input, so the
; flag holds vacuously even at x = inf, where the min returns the constant
; but fract is nan.
...
; ninf on the fsub makes it poison at +/-inf, where its operands are
; infinite, so this can fold.
...
; The input can be neither nan nor infinite, so the nan-avoidant min is
; equivalent to fract with no edge case handling at all.
...
; GFX6-IR-SAME: float nofpclass(nan inf) [[X:%.*]]) #[[ATTR0]] {
; GFX6-IR-NEXT:  [[ENTRY:.*:]]
; GFX6-IR-NEXT:    [[FLOOR:%.*]] = tail call float @llvm.floor.f32(float [[X]])
; GFX6-IR-NEXT:    [[SUB:%.*]] = fsub float [[X]], [[FLOOR]]
; GFX6-IR-NEXT:    [[MIN:%.*]] = tail call float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
...
; IR-FRACT-SAME: float nofpclass(nan inf) [[X:%.*]]) #[[ATTR0]] {
; IR-FRACT-NEXT:  [[ENTRY:.*:]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = call nnan float @llvm.amdgcn.fract.f32(float [[X]])
; IR-FRACT-NEXT:    ret float [[MIN]]
;
...
; No inf check; cannot fold since fract is nan at +/-inf but this returns
; the clamp constant.
...
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub float [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    [[UNO:%.*]] = fcmp ord float [[X]], 0.000000e+00
; IR-FRACT-NEXT:    [[COND:%.*]] = select i1 [[UNO]], float [[MIN]], float [[X]]
; IR-FRACT-NEXT:    ret float [[COND]]
;
...
; IR-LEGALF16-NEXT:    [[SUB:%.*]] = fsub half [[X]], [[FLOOR]]
; IR-LEGALF16-NEXT:    [[MIN:%.*]] = tail call half @llvm.minnum.f16(half [[SUB]], half 9.995110e-01)
...
; IR-LEGALF16-NEXT:    [[SUB:%.*]] = fsub <2 x half> [[X]], [[FLOOR]]
; IR-LEGALF16-NEXT:    [[MIN:%.*]] = tail call <2 x half> @llvm.minnum.v2f16(<2 x half> [[SUB]], <2 x half> splat (half 9.995110e-01))
...
; GFX11-TRUE16:       ; %bb.0: ; %entry
; GFX11-TRUE16-NEXT:    s_waitcnt vmcnt(0) expcnt(0) lgkmcnt(0)
; GFX11-TRUE16-NEXT:    v_floor_f16_e32 v1.h, v0.h
...
; GFX12-TRUE16-NEXT:    s_wait_bvhcnt 0x0
; GFX12-TRUE16-NEXT:    s_wait_kmcnt 0x0
; GFX12-TRUE16-NEXT:    v_floor_f16_e32 v1.h, v0.h
...
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub double [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call double @llvm.minnum.f64(double [[SUB]], double f0x3FEFFFFFFFFFFFFF)
...
; IR-LEGALF16-NEXT:    [[SUB:%.*]] = fsub half [[X]], [[FLOOR]]
; IR-LEGALF16-NEXT:    [[MIN:%.*]] = tail call half @llvm.minnum.f16(half [[SUB]], half 9.995110e-01)
; IR-LEGALF16-NEXT:    [[UNO:%.*]] = fcmp uno half [[X]], 0.000000e+00
; IR-LEGALF16-NEXT:    [[COND:%.*]] = select i1 [[UNO]], half [[X]], half [[MIN]]
...
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub double [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call double @llvm.minnum.f64(double [[SUB]], double f0x3FEFFFFFFFFFFFFF)
; IR-FRACT-NEXT:    [[UNO:%.*]] = fcmp uno double [[X]], 0.000000e+00
; IR-FRACT-NEXT:    [[COND:%.*]] = select i1 [[UNO]], double [[X]], double [[MIN]]
...
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    [[UNO:%.*]] = fcmp uno float [[X]], 0.000000e+00
; IR-FRACT-NEXT:    [[COND:%.*]] = select nsz i1 [[UNO]], float [[X]], float [[MIN]]
; IR-FRACT-NEXT:    ret float [[COND]]
;
...
; IR-FRACT-NEXT:    [[FLOOR:%.*]] = tail call float @llvm.floor.f32(float [[X]])
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub float [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call nsz float @llvm.minnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    [[UNO:%.*]] = fcmp uno float [[X]], 0.000000e+00
; IR-FRACT-NEXT:    [[COND:%.*]] = select i1 [[UNO]], float [[X]], float [[MIN]]
; IR-FRACT-NEXT:    ret float [[COND]]
;
...
; Cannot fold: ninf on the min holds vacuously (its operands are never
; infinite), so it says nothing about x being infinite.
...
; ninf on the fsub stands in for the missing infinity check, so this can
; fold.
...
; IR-FRACT-NEXT:    [[FLOOR:%.*]] = tail call float @llvm.floor.f32(float [[X]])
; IR-FRACT-NEXT:    [[SUB:%.*]] = fsub float [[X]], [[FLOOR]]
; IR-FRACT-NEXT:    [[MIN:%.*]] = tail call float @llvm.minimumnum.f32(float [[SUB]], float f0x3F7FFFFF)
; IR-FRACT-NEXT:    ret float [[MIN]]
;
...
; GFX11-NEXT:    s_cbranch_execnz .LBB63_4
; GFX11-NEXT:  .LBB63_2: ; %UnifiedReturnBlock
; GFX11-NEXT:    s_or_b32 exec_lo, exec_lo, s0
; GFX11-NEXT:    s_setpc_b64 s[30:31]
; GFX11-NEXT:  .LBB63_3: ; %ret
...
; GFX12-NEXT:    s_cbranch_execnz .LBB63_3
; GFX12-NEXT:  ; %bb.1: ; %Flow
; GFX12-NEXT:    s_wait_alu depctr_sa_sdst(0)
; GFX12-NEXT:    s_and_not1_saveexec_b32 s0, s0
; GFX12-NEXT:    s_cbranch_execnz .LBB63_4
; GFX12-NEXT:  .LBB63_2: ; %UnifiedReturnBlock
; GFX12-NEXT:    s_wait_alu depctr_sa_sdst(0)
; GFX12-NEXT:    s_or_b32 exec_lo, exec_lo, s0
; GFX12-NEXT:    s_setpc_b64 s[30:31]
; GFX12-NEXT:  .LBB63_3: ; %ret
...
; GFX12-NEXT:    s_wait_alu depctr_sa_sdst(0)
; GFX12-NEXT:    s_and_not1_saveexec_b32 s0, s0
; GFX12-NEXT:    s_cbranch_execz .LBB63_2
; GFX12-NEXT:  .LBB63_4: ; %edge_cases
