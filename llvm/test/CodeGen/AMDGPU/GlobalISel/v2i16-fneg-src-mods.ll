...
; An fneg whose result is bitcast to <2 x i16> and fed to a packed integer
; operation must be selected as a real xor. It must not be folded into the
; neg_lo/neg_hi modifiers of the VOP3P instruction, which only apply to
; floating-point operations. add/sub/mul and the integer dot products are
; covered in {add,sub,mul}.v2i16.ll and llvm.amdgcn.{s,u}dot2.ll.
