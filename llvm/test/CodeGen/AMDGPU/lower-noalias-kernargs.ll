...
; getUnderlyingObjects does not look through freeze, so the first store's
; underlying object is unknown. It must get no metadata: %f is %out, so
; !noalias {out} here would let AA reorder the two stores.
define amdgpu_kernel void @aliasinfo_unknown_object(ptr addrspace(1) noalias %out) #1 {
; CHECK-LABEL: define amdgpu_kernel void @aliasinfo_unknown_object(
; CHECK-SAME: ptr addrspace(1) noalias [[OUT:%.*]]) #[[ATTR0]] {
; CHECK-NEXT:    [[ALIASINFO_UNKNOWN_OBJECT_KERNARG_SEGMENT:%.*]] = call nonnull align 16 dereferenceable(264) ptr addrspace(4) @llvm.amdgcn.kernarg.segment.ptr()
; CHECK-NEXT:    [[OUT_KERNARG_OFFSET:%.*]] = getelementptr inbounds i8, ptr addrspace(4) [[ALIASINFO_UNKNOWN_OBJECT_KERNARG_SEGMENT]], i64 36
; CHECK-NEXT:    [[OUT_LOAD:%.*]] = load ptr addrspace(1), ptr addrspace(4) [[OUT_KERNARG_OFFSET]], align 4, !invariant.load [[META0]]
; CHECK-NEXT:    [[F:%.*]] = freeze ptr addrspace(1) [[OUT_LOAD]]
; CHECK-NEXT:    store i32 42, ptr addrspace(1) [[F]], align 4
; CHECK-NEXT:    store i32 1, ptr addrspace(1) [[OUT_LOAD]], align 4, !alias.scope [[META60:![0-9]+]]
; CHECK-NEXT:    ret void
;
  %f = freeze ptr addrspace(1) %out
  store i32 42, ptr addrspace(1) %f, align 4
  store i32 1, ptr addrspace(1) %out, align 4
  ret void
}

; A store to an identified global cannot access a noalias argument's
; pointee, so it gets !noalias for every argument's scope.
@gv = addrspace(1) global i32 0

define amdgpu_kernel void @aliasinfo_identified_object(ptr addrspace(1) noalias %out) #1 {
; CHECK-LABEL: define amdgpu_kernel void @aliasinfo_identified_object(
; CHECK-SAME: ptr addrspace(1) noalias [[OUT:%.*]]) #[[ATTR0]] {
; CHECK-NEXT:    [[ALIASINFO_IDENTIFIED_OBJECT_KERNARG_SEGMENT:%.*]] = call nonnull align 16 dereferenceable(264) ptr addrspace(4) @llvm.amdgcn.kernarg.segment.ptr()
; CHECK-NEXT:    [[OUT_KERNARG_OFFSET:%.*]] = getelementptr inbounds i8, ptr addrspace(4) [[ALIASINFO_IDENTIFIED_OBJECT_KERNARG_SEGMENT]], i64 36
; CHECK-NEXT:    [[OUT_LOAD:%.*]] = load ptr addrspace(1), ptr addrspace(4) [[OUT_KERNARG_OFFSET]], align 4, !invariant.load [[META0]]
; CHECK-NEXT:    store i32 7, ptr addrspace(1) @gv, align 4, !noalias [[META63:![0-9]+]]
; CHECK-NEXT:    store i32 1, ptr addrspace(1) [[OUT_LOAD]], align 4, !alias.scope [[META63]]
; CHECK-NEXT:    ret void
;
  store i32 7, ptr addrspace(1) @gv, align 4
  store i32 1, ptr addrspace(1) %out, align 4
  ret void
}
...
; CHECK: [[META61]] = distinct !{[[META61]], [[META62:![0-9]+]], !"out"}
; CHECK: [[META62]] = distinct !{[[META62]], !"aliasinfo_unknown_object"}
; CHECK: [[META63]] = !{[[META64:![0-9]+]]}
; CHECK: [[META64]] = distinct !{[[META64]], [[META65:![0-9]+]], !"out"}
; CHECK: [[META65]] = distinct !{[[META65]], !"aliasinfo_identified_object"}
;.
