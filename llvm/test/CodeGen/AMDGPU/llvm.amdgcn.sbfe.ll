...
; The i64 intrinsic with a constant width used to crash SelectionDAG: widths
; that are multiples of 32 hit a RAUW type mismatch in the BFE combine (which
; masks the width with 0x1f and replaces the i64 node with an i32 zero), other
; constant widths hit a 32-bit demanded-bits query on the i64 operand, and
; non-constant widths failed to select.
