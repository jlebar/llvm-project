...
; The merged sincos call is inserted after the argument's definition. When the
; argument is a phi that is not the last phi in its block, the call must go
; after the whole phi group.
