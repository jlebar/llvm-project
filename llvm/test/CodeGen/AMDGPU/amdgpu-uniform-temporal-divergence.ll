...
; The ballot argument is uniform inside the loop, so (icmp eq %ballot, 0) is
; rewritten to a NOT of the argument. That NOT is defined inside the loop, and
; each lane captures its value from the iteration in which it exits, so a
; second ballot of it after the loop is a ballot of a divergent value and must
; not be folded.
...
; PASS-CHECK-SAME: ptr addrspace(1) [[OUT:%.*]], ptr addrspace(1) [[DATA:%.*]]) #[[ATTR0]] {
; PASS-CHECK-NEXT:  [[ENTRY:.*]]:
; PASS-CHECK-NEXT:    [[TID:%.*]] = call i32 @llvm.amdgcn.workitem.id.x()
; PASS-CHECK-NEXT:    br label %[[H:.*]]
; PASS-CHECK:       [[H]]:
...
; COMB-CHECK-SAME: ptr addrspace(1) [[OUT:%.*]], ptr addrspace(1) [[DATA:%.*]]) #[[ATTR0]] {
; COMB-CHECK-NEXT:  [[ENTRY:.*]]:
; COMB-CHECK-NEXT:    [[TID:%.*]] = call i32 @llvm.amdgcn.workitem.id.x()
; COMB-CHECK-NEXT:    br label %[[H:.*]]
; COMB-CHECK:       [[H]]:
...
; Same, but the icmp user of the first ballot is outside the loop, so the NOT
; is created outside the loop with a temporally divergent operand. The second
; ballot of it must not be folded.
...
; Readlane of a value derived from the NOT after the loop must not be folded
; either. The zext is an original instruction that uniformity info itself
; knows is temporally divergent here.
...
; Same as ballot_user_outside, with a readlane of the NOT. The old side table
; marked the NOT uniform and folded the readlane to it, even though the NOT is
; temporally divergent. Uniformity info rejects the fold: values it did not
; analyze are conservatively divergent.
...
; PASS-CHECK-NEXT:    br i1 [[DIV_EXITX]], label %[[EXIT:.*]], label %[[H]]
; PASS-CHECK:       [[EXIT]]:
; PASS-CHECK-NEXT:    [[TMP1:%.*]] = xor i1 [[X]], true
; PASS-CHECK-NEXT:    [[NONE:%.*]] = icmp eq i32 [[BALLOT]], 0
; PASS-CHECK-NEXT:    [[TMP0:%.*]] = call i1 @llvm.amdgcn.readlane.i1(i1 [[TMP1]], i32 0)
; PASS-CHECK-NEXT:    [[EXT:%.*]] = zext i1 [[TMP0]] to i32
; PASS-CHECK-NEXT:    [[OG:%.*]] = getelementptr i32, ptr addrspace(1) [[OUT]], i32 [[TID]]
...
; Without temporal divergence the (icmp eq %ballot, 0) fold still applies in
; straight-line code. The second ballot is not folded in the same run because
; the NOT is new to this run's uniformity info; a later run of this pass (with
; recomputed uniformity) folds it. instcombine alone does not -- it only sinks
; the NOT into the compare, so COMB-CHECK still has the second ballot.
...
; COMB-CHECK-SAME: i32 [[V:%.*]], ptr addrspace(1) [[OUT:%.*]]) #[[ATTR0]] {
; COMB-CHECK-NEXT:  [[ENTRY:.*:]]
; COMB-CHECK-NEXT:    [[X:%.*]] = icmp slt i32 [[V]], 1
; COMB-CHECK-NEXT:    [[BALLOT2:%.*]] = call i32 @llvm.amdgcn.ballot.i32(i1 [[X]])
; COMB-CHECK-NEXT:    [[ANY:%.*]] = icmp ne i32 [[BALLOT2]], 0
