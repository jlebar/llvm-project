...
; The domain-error NaN for |x| > 1 is quiet, so no-snan sources still
; rule out snan.
define float @ret_asin_nosnan(float nofpclass(snan) %arg) {
; CHECK-LABEL: define nofpclass(snan inf) float @ret_asin_nosnan
; CHECK-SAME: (float nofpclass(snan) [[ARG:%.*]]) #[[ATTR1]] {
; CHECK-NEXT:    [[CALL:%.*]] = call nofpclass(snan inf) float @llvm.asin.f32(float nofpclass(snan) [[ARG]]) #[[ATTR2]]
; CHECK-NEXT:    ret float [[CALL]]
;
  %call = call float @llvm.asin.f32(float %arg)
  ret float %call
...
; The domain-error NaN for |x| > 1 is quiet, so no-snan sources still
; rule out snan.
define float @ret_acos_nosnan(float nofpclass(snan) %arg) {
; CHECK-LABEL: define nofpclass(snan inf nzero nsub nnorm) float @ret_acos_nosnan
; CHECK-SAME: (float nofpclass(snan) [[ARG:%.*]]) #[[ATTR1]] {
; CHECK-NEXT:    [[CALL:%.*]] = call nofpclass(snan inf nzero nsub nnorm) float @llvm.acos.f32(float nofpclass(snan) [[ARG]]) #[[ATTR2]]
; CHECK-NEXT:    ret float [[CALL]]
;
  %call = call float @llvm.acos.f32(float %arg)
  ret float %call
