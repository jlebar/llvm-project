...
; Casting flat null to a smaller addrspace does not necessarily result in null
; again (AMDGPU maps flat null to local -1), so the mask must stay in the flat
; addrspace.
...
; Test some align-down patterns. These only touch the low bits, which are
; preserved through the cast, but the pointer may still be flat null (whose
; local representation, -1, is not preserved by masking), so the mask has to
; stay in the flat addrspace.
...
; ptrmask(flat null, -4096) is flat null, but the local representation of flat
; null is -1 and -1 & -4096 is not -1, so the align-down must not be performed
...
; If the flat pointer is known non-null, the mask can be applied to the local
; pointer.
