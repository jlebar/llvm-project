...
protected:
  ...
  /// Called by SelectCodeCommon when \p N matches no pattern in the matcher
  /// table. The default implementation reports a fatal error; a target that
  /// overrides this to diagnose-and-continue must replace all of \p N's
  /// values and remove it from the DAG before returning.
  virtual void CannotYetSelect(SDNode *N);
