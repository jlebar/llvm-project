...
bool AMDGPUPreLegalizerCombinerImpl::matchClampI64ToI16(
  ...
  // Cmp1 is the outer bound, Cmp2 the inner one. The lower bound must not
  // exceed the upper bound: smin(smax(X, Lo), Hi) with Lo > Hi is constant
  // Hi (and smax(smin(X, Hi), Lo) with Lo > Hi is constant Lo), while the
  // med3 the combine builds would still clamp X to [Hi, Lo].
  auto IsApplicableForCombine = [&MatchInfo](bool OuterIsMin) -> bool {
    ...
    // Check that both comparison values are between SHORT_MIN and SHORT_MAX.
    // This also makes the Hi - Lo below free of overflow.
