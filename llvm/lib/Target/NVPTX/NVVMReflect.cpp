...
// Argument of reflect call to retrieve precise division mode
#define CUDA_PREC_DIV_NAME "__CUDA_PREC_DIV"
// Name of module metadata where precise division mode is stored
#define CUDA_PREC_DIV_MODULE_NAME "nvvm-reflect-prec-div"
...
class NVVMReflect {
  ...
  // with. Should include __CUDA_FTZ, __CUDA_PREC_DIV and __CUDA_ARCH values.
...
public:
  // __CUDA_FTZ and __CUDA_PREC_DIV are assigned in `runOnModule` by checking
  // the nvvm-reflect-ftz and nvvm-reflect-prec-div module metadata.
  // __CUDA_PREC_DIV defaults to 1: unless a frontend requests the approximate
  // division, libdevice should use the IEEE round-to-nearest variant, which
  // is both nvcc's default (-prec-div=true) and how the backend lowers f32
  // fdiv by default.
  explicit NVVMReflect(unsigned SmVersion)
      : ReflectMap({{CUDA_ARCH_NAME, SmVersion * 10},
                    {CUDA_PREC_DIV_NAME, 1}}) {}
...
// precedence over initial values (i.e. __CUDA_FTZ and __CUDA_PREC_DIV from
...
// __CUDA_PREC_DIV from module metadata, and then the key/value pairs from the
// command line.
void NVVMReflect::populateReflectMap(Module &M) {
  ...
  if (auto *Flag = mdconst::extract_or_null<ConstantInt>(
          M.getModuleFlag(CUDA_PREC_DIV_MODULE_NAME)))
    ReflectMap[CUDA_PREC_DIV_NAME] = Flag->getSExtValue();
