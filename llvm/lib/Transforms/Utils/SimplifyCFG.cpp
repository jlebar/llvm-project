...
static void hoistConditionalLoadsStores(
  ...
  for (auto *I : SpeculatedConditionalLoadsStores) {
    ...
    if (auto *LI = dyn_cast<LoadInst>(I)) {
      ...
      // When the mask is false the masked load returns PassThru rather than a
      // loaded value, so the load's !range only describes the call's result
      // after widening it to cover PassThru. A non-constant PassThru can hold
      // anything; drop the range in that case.
